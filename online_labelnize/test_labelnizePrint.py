...
desired_caps = {
    ...
    "appium:appPackage": "com.labelnize.printer",
...
back = ['XPATH',
        '//android.widget.ImageView[@resource-id="com.labelnize.printer:id/iv_back"]']  # 返回：如果进入首页自动连接，则‘返回’，无需再次进行连接
refresh = ['XPATH', '//android.widget.TextView[@text="刷新"]']  # 连接页面的刷新
disconnect = '//android.widget.TextView[@resource-id="com.labelnize.printer:id/act_two_inch_bt_disconnect_tv"]'  # 取消连接：如果出现‘取消连接’，则说明已经自动连接，无需再次连接
connectFail = '//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_message"]'  # 请尝试通过 BLE 搜索连接：连接失败（P21、P31S）晶振问题
sure = ['XPATH', '//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_sure"]']  # 是否切换BLE连接的确认按钮
...
class online(unittest.TestCase):
    ...
    def first_open(self):
        ...
        if self.is_element_present('//android.widget.TextView[@resource-id="com.labelnize.printer:id/connect_later_tv"]'):
            ...
            confirm = self.driver.find_element_by_xpath(
                '//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_ok"]')
            confirm.click()
            # 跳过
            skip = self.driver.find_element_by_xpath(
                '//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_right"]')
            skip.click()
    ...
    def openEditor(self):
        print("\n打开自定义模板功能开始测试\n")
        editor = ['XPATH',
                  '(//android.view.ViewGroup[@resource-id="com.labelnize.printer:id/cl_content"])[1]/android.widget.ImageView[2]']
    ...
    def test_print(self, devicesName, devicesBuletooth):
        ...
        connectTitle = self.findElement(
            ['XPATH', '//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_title"]'])  # 安卓的双击连接对打印机排序
        self.double_tap_element(connectTitle)  # 双击连接对打印机排序
        self.double_tap_element(connectTitle)  # 双击连接对打印机排序
        if not self.is_element_present(
                f'//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_ble_address" and @text="{value}"]'):  # 如果没有找到对应的蓝牙地址连接
            while True:  # 一直循环找对应的蓝牙地址
                ...
                if self.is_element_present(
                        f'//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_ble_address" and @text="{value}"]'):  # 如果找到了就退出循环
                    break
        self.click(['XPATH',
                    f'//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_ble_address" and @text="{value}"]'])  # 找到了就点击蓝牙进行连接
        ...
        if self.is_element_present(connectFail):  # 如果连接失败（P21、P31S）晶振问题
            ...
            if not self.is_element_present(  # 与上面一致
                    f'//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_ble_address" and @text="{value}"]'):
                while True:
        ...
        deviceName = self.findElement(
            ['XPATH', '//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_printer_model"]'])
        connectStatus = self.findElement(
            ['XPATH', '//android.widget.TextView[@resource-id="com.labelnize.printer:id/tv_connect"]'])
        ...
        self.driver.find_element_by_xpath(
            '//android.widget.EditText[@resource-id="com.labelnize.printer:id/etAmount"]').clear()  # 清空打印的份数（默认是1）
        time.sleep(2)
        self.driver.find_element_by_xpath(
            '//android.widget.EditText[@resource-id="com.labelnize.printer:id/etAmount"]').send_keys(
            "5")  # 清空后输入份数为5
        self.click(['XPATH',
...
runner = TestRunner(suite,
                    filename="Labelnize自动化测试报告.html",
                    report_dir=r"C:\Users\YZY\Desktop\nelko\online_labelnize\report",  # 放桌面
                    title='Labelnize版本上线测试报告',
                    tester='欢',
                    desc="测试",
